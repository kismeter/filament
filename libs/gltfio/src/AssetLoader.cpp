...
bool FAssetLoader::createPrimitive(const cgltf_primitive* inPrim, Primitive* outPrim,
    ...
    // If an ubershader is used then we provide a single dummy buffer for all required vertex
    // attributes. The color data should be a sequence of normalized UBYTE4, so the dummy tex coords
    // are USHORT2 to make the sizes match.
    int dummySlot = -1;
